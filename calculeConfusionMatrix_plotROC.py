...
parser = argparse.ArgumentParser(description='Calcule confusion matrix and plot ROC curve, from orginal image vs binarized dolphin image. Use the dolphin polygon annotation to delimit true condition positive.Crop and save it as .png image')
...
print "***************"
FPR, TPR, thresholds = roc_curve(mask_real_dolphin.flatten(), mask_predict_dolphin.flatten(), pos_label=True)
roc_auc = auc(FPR, TPR)
...
plt.show()
fn_roc="{}{}".format(os.path.splitext(args.binarized_im)[0],"_ROC.png")
fig.savefig(fn_roc, dpi=100)
